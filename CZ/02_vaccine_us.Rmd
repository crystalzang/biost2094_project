...
table(vaccine_us_long$status)

ggplot(data = filter(us_states, population == "18+", status== "fully vaccinated"),
            aes(x = long, y = lat,
...
ggplot(data = filter(us_states, brand == "Pfizer"),
            aes(x = long, y = lat,
                group = group, fill = percent))+
  geom_polygon(color = "gray90", size = 0.1) +
    coord_map(projection = "albers", lat0 = 39, lat1 = 45)+
  labs(title=paste0("Vaccination Stuatus in the U.S. By ", "Pfizer"),
        x =" ", y = " ")
```
