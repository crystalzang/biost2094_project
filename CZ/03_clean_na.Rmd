...
vaccine_final <- vaccine_final%>%
  ...
  mutate(total_vaccine_per_pop = total_vaccine/population*100)
...
combine <-combine %>%
  select(country,date, population, daily_vaccinations,continent,people_vaccinated)%>%
  group_by(country)%>%
  arrange(date)%>%
  mutate(daily_vaccinations = if_else(is.na(daily_vaccinations ), 0, daily_vaccinations))%>%
  group_by(country)%>%
...
combine_new <- combine%>%
  ...
  filter(country %notin% countries_0)
#if values are 0, fill in as na
combine_new$people_vaccinated <-na_if(combine_new$people_vaccinated,0)
