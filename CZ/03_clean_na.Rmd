...
vaccine_final <- gather(s, key = "date", value = "total_people_vaccinated",-country)

library(mosaic)
vaccine_final$date <- as.Date(vaccine_final$date,"%Y/%m/%d")

ggplot(data = filter(vaccine_final, country=="United States"), aes(x = date, y = total_people_vaccinated)) + 
  geom_line()  +
  aes(colour = country) + 
